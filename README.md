...
[![NPM version](https://img.shields.io/npm/v/@unifyroom/insight-api.svg)](https://npmjs.org/package/@unifyroom/insight-api)
...
A Unifyroom blockchain REST and WebSocket API Service
...
- [API HTTP Endpoints](#api-http-endpoints)
    ...
    - [Status of the Unifyroom Network](#status-of-the-dash-network)
...
```bash
npm install -g @unifyroom/dashcore-node
dashcore-node create mynode
cd mynode
dashcore-node install @unifyroom/insight-api
...
- [Unifyroomcore Node Unifyroom 6.x](https://github.com/unifyroom/dashcore-node)

**Note:** You can use an existing Unifyroom data directory, however `txindex`, `addressindex`, `timestampindex` and `spentindex` need to be enabled in `dash.conf`, as well as a few other additional fields.
...
### Status of the Unifyroom Network
...
- (Medium)[How to setup a Unifyroom Instant-Send Transaction using Insight API - The comprehensive way](https://medium.com/@obusco/setup-instant-send-transaction-the-comprehensive-way-a80a8a0572e)
...
[MIT](LICENSE) &copy; Unifyroom Core Group, Inc.
